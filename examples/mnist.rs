...
use ml_lib::{Tensor, layer, optimizer, loss};
...
fn main() {
    ...
    let (inputs, targets) = load_mnist_as_tensors(
        images,
        labels,
        10,
    ).expect("Failed to load MNIST dataset.");

    println!("First input stats:");
    let x = &inputs[0];
    println!("shape = {:?}, min = {}, max = {}, mean = {}",
             x.shape,
    ...
    println!("shape = {:?}, data = {:?}", targets[0].shape, targets[0].data);
    ...
    layer::Sequential::train(&model, &mut opt, &inputs, &targets, 2, 64, loss::softmax_cross_entropy);
}
